...
void IntSet::grow(){
    int *tmp = new int[sizeElts + 1]; // enlarge the size of array by one
    for (int i = 0; i < numElts; i++) {
        tmp[i] = elts[i];
    }
    delete [] elts; // delete the memory that elts pointing to,
    // but the pointer elts is the object of this class, would
    elts = tmp;
    sizeElts += 1;
}
